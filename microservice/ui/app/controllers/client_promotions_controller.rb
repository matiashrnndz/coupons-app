class ClientPromotionsController < ApplicationController
  ...
  def new_token
    response = @request_service.get_request('get',"/promotions/new_token", {}, session['token'])
    if response[:connection_error]
      ...
      if response['errors'] && response['errors'].count > 0
        ...
        @promotion_definitions = response['promotion_definitions']['data']
      end
    end
  end

  def generate_token
    request_params = { codes: params[:token][:codes], name: params[:token][:name] }
    response = @request_service.get_request('post',"/promotions/generate_token", request_params, session['token'])
    if response[:connection_error]
      render file: "#{Rails.root}/public/500", layout: false, status: :error
    else
      if response['errors'] && response['errors'].count > 0
